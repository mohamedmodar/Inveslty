...
class ModelNN(ModelPipeline):
    
    def __init__(self, tag, target):  
        super().__init__(tag, target, "NN")
                        ...
                        for batch_size in params["batch_size"]:
                            ...
                            with mlflow.start_run(nested=True, run_name=data_params_str):
                                ...
                                model, metrics = self.run_grid_search_model(
                                    current_model_params,
                                    X_train, X_train_main, y_train_main, X_val, y_val
                                )
    ...
    def set_grid_search_params(self):
        return {
            "nn_units": [8, 16],  # Reduced from [32, 64, 128] # [8, 16],
            "dropout_rate": [0.2],  # Reduced from [0.1, 0.2, 0.3]
            "learning_rate": [0.01],  # Reduced from [0.001, 0.01, 0.1]
            "epochs": [50],
            "batch_size": [8]   # Reduced from [16, 32, 64] # [4, 8] 
        }
    ...
    def run_model(self, X_train, X_test, y_train, y_test, params):
        with mlflow.start_run():
            ...
            # train model
            history = self.train_model(model, X_train, y_train, X_test, y_test, params)
